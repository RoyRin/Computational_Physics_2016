...
def fFx(k,n, y):
    ...
    return -k *m*(y[1])**(n) *  (xvel/totalVel)
def fFy(k,n,y):
    m=1.
    yvel = y[3]
    totalVel = (y[1]**2 + y[3]**2)**0.5
    return -k  *m* (y[3])**((n)) *  (yvel/totalVel)
...
ballsAir = rk4_Balls(funcBallsFallingAir, initValue,t1, k, nval[2])
...
plt.title( "Plot of max distance versus angle for n = %f" %nval[3] )
