...
pedestal_top = vec(-1,hpedestal/2,0) # top of inner bar of U-shaped upper assembly
theta1 = 1.6*pi/2.0 # initial upper angle (from vertical)
theta1dot = 0 # initial rate of change of theta1
theta2 = 0.9 # initial lower angle (from vertical)
theta2dot = 0 # initial rate of change of theta2

pedestal2_top = vec(1,hpedestal/2,0) # top of inner bar of U-shaped upper assembly
theta12 = theta1*1.001 # initial upper angle (from vertical)
theta1dot2 = 0 # initial rate of change of theta1
theta22 = theta2 # initial lower angle (from vertical)
theta2dot2 = 0 # initial rate of change of theta2
...
bar2 = box( pos=axle2.pos+vec(L2display/2-d/2,0,(gap+d)/2), size=vec(L2display,d,d), color=color.green )
...
pedestal2 = box( pos=pedestal2_top-vec(0,hpedestal/2,offset),
                size=vec(wpedestal,1.1*hpedestal,wpedestal),
                color=vec(0.4,0.4,0.5) )
base2 = box( pos=pedestal2_top-vec(0,hpedestal+tbase/2,offset),
                 size=vec(wbase,tbase,wbase),
                 color=pedestal.color )
axle12 = cylinder( pos=pedestal2_top-vec(0,0,gap/2-d/4), axis=vec(0,0,-1),
                 size=vec(offset,d/4,d/4), color=color.yellow )

bar12 = box( pos=pedestal2_top+vec(L1display/2-d/2,0,-(gap+d)/2), 
                 size=vec(L1display,d,d), color=color.red )
bar12.rotate( angle=-pi/2, axis=vec(0,0,1), origin=vec(axle12.pos.x, axle12.pos.y, bar12.pos.z) )
bar12.rotate( angle=theta12, axis=vec(0,0,1), origin=vec(axle12.pos.x, axle12.pos.y, bar12.pos.z) )

bar1b2 = box( pos=pedestal2_top+vec(L1display/2-d/2,0,(gap+d)/2), 
                 size=vec(L1display,d,d), color=bar1.color )
bar1b2.rotate( angle=-pi/2, axis=vec(0,0,1), origin=vec(axle12.pos.x, axle12.pos.y, bar1b2.pos.z) )
bar1b2.rotate( angle=theta12, axis=vec(0,0,1), origin=vec(axle12.pos.x, axle12.pos.y, bar1b2.pos.z) )

pivot12 = vec(axle12.pos.x, axle12.pos.y, 0)

axle22 = cylinder( pos=pedestal2_top+vec(L1,0,-(gap+d)/2), axis=vec(0,0,1), 
                size=vec(gap+d,axle12.size.y/2,axle12.size.y/2), color=axle12.color )
axle22.rotate( angle=-pi/2, axis=vec(0,0,1), origin=vec(axle12.pos.x, axle12.pos.y, axle22.pos.z) )
axle22.rotate( angle=theta12, axis=vec(0,0,1), origin=vec(axle12.pos.x, axle12.pos.y, axle22.pos.z) )

bar22 = box( pos=axle22.pos+vec(L2display/2-d/2,0,(gap+d)/2), size=vec(L2display,d,d), color=color.green )
bar22.rotate( angle=-pi/2,  axis=vec(0,0,1), origin=vec(axle22.pos.x, axle22.pos.y, bar22.pos.z) )
bar22.rotate( angle=theta22,  axis=vec(0,0,1), origin=vec(axle22.pos.x, axle22.pos.y, bar22.pos.z) )
...
while True:
    ...
    # Calculate accelerations of the Lagrangian coordinates=
    atheta12 = ((E*C/B)*sin(theta12)-F*sin(theta22))/(D-E*A/B)
    atheta22 = -(A*atheta12+C*sin(theta12))/B
    # Update velocities of the Lagrangian coordinates=
    theta1dot2 = theta1dot2+atheta12*dt
    theta2dot2 = theta2dot2+atheta22*dt
    # Update Lagrangian coordinates=
    dtheta12 = theta1dot2*dt
    dtheta22 = theta2dot2*dt
    theta12 = theta12+dtheta12
    theta22 = theta22+dtheta22
    
    bar12.rotate( angle=dtheta12, axis=vec(0,0,1), origin=pivot12 )
    bar1b2.rotate( angle=dtheta12, axis=vec(0,0,1), origin=pivot12 )
    pivot22 = vec(axle22.pos.x, axle22.pos.y, pivot12.z)
    axle22.rotate( angle=dtheta12, axis=vec(0,0,1), origin=pivot12 )
    bar22.rotate( angle=dtheta22, axis=vec(0,0,1), origin=pivot22 )
    pivot22 = vec(axle22.pos.x, axle22.pos.y, pivot12.z)
    bar22.pos = pivot22 + bar22.axis/2
