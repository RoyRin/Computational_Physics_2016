...
Nx = 101;        Nt = 7500;     Dx = 0.03;     Dt = 0.9                                                              
KAPPA = 210.; SPH = 900.; RHO = 2700. # conductivity, specf heat, density                                                      
T = zeros( (Nx, 2), float);  Tpl = zeros( (Nx, 31000), float)  
...
for t in range (1, Nt):                                  # time iteration
   for ix in range (1, Nx - 1):                      # Finite differences
      T[ix, 1] = T[ix, 0] +  cons*(T[ix+1, 0] + T[ix-1, 0] - 2.*T[ix,0]) - 1e-8 *pow(T[ix,0],4)                                                       
...
y = list(range(1, 300))                      # every 10 points in y (time)
